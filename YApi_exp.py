...
def Yapi_vuln(vul_url,command):
    ...
    # s = requests.session()
    # login_reps= s.post(url=login_url,data=login_data,verify=False)
    # print(login_reps.text)
    # print(login_reps.cookies)
    #
    get_user_info = s.get(url=user_info)
    print(get_user_info.text)

    # get_user_info = requests.get(url=user_info,hearder=login_succerss_cookies)
    # print(get_user_info.text)
...
def reg_status(url):
    try:
        ...
        return reg_statu
        # aa=reps_reg_status.json()["canRegister"]
        # print(aa)
    except:
        print("网络连接超时")
        sys.exit()
